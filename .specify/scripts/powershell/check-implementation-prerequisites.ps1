...
foreach ($arg in $Arguments) {
    ...
    elseif ($arg -eq "--help" -or $arg -eq "-h") {
        Write-Host "Usage: .\check-implementation-prerequisites.ps1 [<feature-name>] [--json]"
...
if ($jsonMode) {
    ...
    if (Test-Path $tasks) { $docs += "tasks.md" }
    ...
    Write-Output (Test-FileExists -Path $tasks -Label "tasks.md")
}
